# -*- coding: utf-8 -*-
# Generated by Django 1.11.21 on 2019-06-21 05:50
from __future__ import unicode_literals

from django.db import migrations, models
import django.db.models.deletion
import job.models


class Migration(migrations.Migration):

    dependencies = [
        ('job', '0023_auto_20190621_0441'),
    ]

    operations = [
        migrations.AddField(
            model_name='student',
            name='Degree_year',
            field=models.IntegerField(choices=[(1984, 1984), (1985, 1985), (1986, 1986), (1987, 1987), (1988, 1988), (1989, 1989), (1990, 1990), (1991, 1991), (1992, 1992), (1993, 1993), (1994, 1994), (1995, 1995), (1996, 1996), (1997, 1997), (1998, 1998), (1999, 1999), (2000, 2000), (2001, 2001), (2002, 2002), (2003, 2003), (2004, 2004), (2005, 2005), (2006, 2006), (2007, 2007), (2008, 2008), (2009, 2009), (2010, 2010), (2011, 2011), (2012, 2012), (2013, 2013), (2014, 2014), (2015, 2015), (2016, 2016), (2017, 2017), (2018, 2018), (2019, 2019)], default=job.models.current_year, null=True, verbose_name='year'),
        ),
        migrations.AddField(
            model_name='student',
            name='Master_Degree_College',
            field=models.ForeignKey(null=True, on_delete=django.db.models.deletion.SET_NULL, related_name='Master_Degree_College', to='job.College'),
        ),
        migrations.AlterField(
            model_name='student',
            name='Degree_College',
            field=models.ForeignKey(null=True, on_delete=django.db.models.deletion.SET_NULL, related_name='Degree_College', to='job.College'),
        ),
    ]
